(function(){
    const mobileErrorElement = document.getElementById('mobile-error') as HTMLElement
    const countryCodeInput = document.getElementById('country_code') as HTMLInputElement
    const mobileNumberInput = document.getElementById('mobile_number') as HTMLInputElement
    ...
    ;(document.getElementById('start_whatsapp') as HTMLElement).addEventListener('click', () => {
        ...
        const textMessage = encodeURIComponent((document.getElementById('text_message') as HTMLInputElement).value || '')
        window.open(`https://wa.me/${filteredCountryCode}${filteredNumber}?text=${textMessage}`, '_blank')
    })
    ;(document.getElementById('facebook_share') as HTMLElement).addEventListener('click', () => {
        shareWindow(
            `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}&t=${document.title}`
        )
    })
    ;(document.getElementById('twitter_share') as HTMLElement).addEventListener('click', () => {
